...
def all_movie():
    ...
    with open("data/redial/spider_result/movie_id_name_fre.jsonl", 'w', encoding='utf-8') as f:
...
def all_genre():
    ...
    with open('./data/redial/statistic/movie_statistic_genre.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有 {} 部电影含有类别属性\n".format(has_genre))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_tails.txt', 'a', encoding='utf-8') as f:
        for item in genre_statistic:
            if item[0] == 'no_genre':
                continue
            if item[1] > 1:
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_genre.txt', 'w', encoding='utf-8') as f:
        for item in genre_statistic:
            if item[0] == 'no_genre':
                continue
            if item[1] > 1:
...
def all_language():
    ...
    with open('./data/redial/statistic/movie_statistic_language.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有语言的电影数量：{} \n".format(has))
        f.write("无语言的电影数量：{}\n".format(hasnot))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_language.txt', 'w', encoding='utf-8') as f:
        for item in statistic:
            if item[0] == 'no_language':
                continue
            if item[1] > 1:
...
def all_director():
    ...
    with open('./data/redial/statistic/movie_statistic_director.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有导演的电影数量：{} \n".format(has))
        f.write("无导演的电影数量：{}\n".format(hasnot))
        f.write("共有 {} 位导演\n".format(len(statistic.keys())))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_director.txt', 'w', encoding='utf-8') as f:
        for item in statistic:
            if item[1] > 1:
                if item[0] == 'no_director':
                    continue
...
def all_starring():
    ...
    with open('./data/redial/statistic/movie_statistic_starring.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有主演的电影数量：{} \n".format(has))
        f.write("无主演的电影数量：{}\n".format(hasnot))
        f.write("共有 {} 位主演\n".format(len(statistic.keys())))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_starring.txt', 'w', encoding='utf-8') as f:
        for item in statistic:
            if item[1] > 1:
                if item[0] == 'no_starring':
                    continue
...
def all_producer():
    ...
    with open('./data/redial/statistic/movie_statistic_producer.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有制片的电影数量：{} \n".format(has))
        f.write("无制片的电影数量：{}\n".format(hasnot))
        f.write("共有 {} 位制片\n".format(len(statistic.keys())))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_producer.txt', 'w', encoding='utf-8') as f:
        for item in statistic:
            if item[1] > 1:
                if item[0] == 'no_producer':
                    continue
...
def all_writer():
    ...
    with open('./data/redial/statistic/movie_statistic_writer.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有编剧的电影数量：{} \n".format(has))
        f.write("无编剧的电影数量：{}\n".format(hasnot))
        f.write("共有 {} 位编剧\n".format(len(statistic.keys())))
    ...
    # xx 的电影超过1 才算有效xx
    with open('data/redial/statistic/valid_writer.txt', 'w', encoding='utf-8') as f:
        for item in statistic:
            if item[1] > 1:
                if item[0] == 'no_writer':
                    continue
...
def all_img():
    ...
    with open('./data/redial/statistic/movie_statistic_img.jsonl', 'w', encoding='utf-8') as f:
        f.write("共有 {} 部电影\n".format(movie_count))
        f.write("有海报的电影数量：{} \n".format(has))
        f.write("无海报的电影数量：{}\n".format(hasnot))
...
def statistic_movie_info():
    ...
    :return:
    """
    if os.path.exists('data/redial/statistic/valid_tails.txt'):
        os.remove('data/redial/statistic/valid_tails.txt')
    all_genre()
    all_language()
    all_director()
    all_starring()
    ...
    valid_name = set()
    with open('data/redial/statistic/valid_director.txt', 'r', encoding='utf-8') as inf:
        valid_director = inf.read().splitlines()
    with open('data/redial/statistic/valid_producer.txt', 'r', encoding='utf-8') as inf:
        valid_producer = inf.read().splitlines()
    with open('data/redial/statistic/valid_writer.txt', 'r', encoding='utf-8') as inf:
        valid_writer = inf.read().splitlines()
    with open('data/redial/statistic/valid_starring.txt', 'r', encoding='utf-8') as inf:
        valid_starring = inf.read().splitlines()
    with open('./data/redial/statistic/valid_genre.txt', 'r', encoding='utf-8') as f:
        valid_genre = f.read().splitlines()
    with open('./data/redial/statistic/valid_language.txt', 'r', encoding='utf-8') as f:
    ...
    with open('data/redial/statistic/valid_all_entity.pkl', 'wb') as outf:
        pkl.dump(valid_all_entity, outf)

    if os.path.exists('data/redial/statistic/valid_director.txt'):
        os.remove('data/redial/statistic/valid_director.txt')

    if os.path.exists('data/redial/statistic/valid_producer.txt'):
        os.remove('data/redial/statistic/valid_producer.txt')

    if os.path.exists('data/redial/statistic/valid_writer.txt'):
        os.remove('data/redial/statistic/valid_writer.txt')

    if os.path.exists('data/redial/statistic/valid_starring.txt'):
        os.remove('data/redial/statistic/valid_starring.txt')

    if os.path.exists('data/redial/statistic/valid_genre.txt'):
        os.remove('data/redial/statistic/valid_genre.txt')

    if os.path.exists('data/redial/statistic/valid_language.txt'):
        os.remove('data/redial/statistic/valid_language.txt')
...
if __name__ == '__main__':
    ...
    # statistic_kg()
    zinc = pd.DataFrame([[1, 1, 1], [2, 2, 2], [3, 3, 3]], columns=['MWTT', 'LOGP', 'CANONICAL_SMILES'])
    # df = pd.DataFrame([[4, 4, 4]])
    # df.columns = ['MWTT', 'LOGP', 'CANONICAL_SMILES']
    print(zinc[['MWTT', 'LOGP', 'CANONICAL_SMILES']])
    # print(zinc[(zinc['MWT'] > 1) & (zinc['MWT'] <= 3)])
